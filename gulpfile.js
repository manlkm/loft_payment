...
gulp.task('html', function() {
  ...
  gulp.src(['src/html/**/*.html','src/templates/**/*.html'])
  .pipe(replace('<!-- inject:appTitle -->', config.appTitle))
  .pipe(gulp.dest(config.dest));
});
...
gulp.task('js', function() {
    streamqueue({ objectMode: true },
      ...
      gulp.src('./configs/'+config.environment+'.js'),
      gulp.src(['src/templates/**/*.html']).pipe(templateCache({ module: 'LoftPayment' }))
    )
